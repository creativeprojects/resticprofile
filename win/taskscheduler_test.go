...
func TestTaskSchedulerConversion(t *testing.T) {
	...
	t.Logf("%+v", task.Triggers)
	// first task should be a single event
	trigger, ok := task.Triggers[0].(taskmaster.TimeTrigger)
	require.True(t, ok)
	assert.Equal(t, "2020-01-01 00:00:00", trigger.StartBoundary.Format("2006-01-02 15:04:05"))
}
