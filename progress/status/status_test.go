...
func TestBackupSuccess(t *testing.T) {
	...
	status.Profile(profileName).BackupSuccess(progress.Summary{Duration: parseDuration("2h45m")}, "")
...
func TestBackupError(t *testing.T) {
	...
	status := NewStatus("")
	assert.Nil(t, status.Profile(profileName).Backup)
	status.Profile(profileName).BackupError(errors.New(errorMessage), progress.Summary{Duration: parseDuration("45s")}, "")
...
func TestRetentionSuccess(t *testing.T) {
	...
	status.Profile(profileName).RetentionSuccess(progress.Summary{Duration: parseDuration("2h45m")}, "")
...
func TestRetentionError(t *testing.T) {
	...
	status := NewStatus("")
	assert.Nil(t, status.Profile(profileName).Retention)
	status.Profile(profileName).RetentionError(errors.New(errorMessage), progress.Summary{Duration: parseDuration("45s")}, "")
...
func TestCheckSuccess(t *testing.T) {
	...
	status.Profile(profileName).CheckSuccess(progress.Summary{Duration: parseDuration("2h45m")}, "")
...
func TestCheckError(t *testing.T) {
	...
	status := NewStatus("")
	assert.Nil(t, status.Profile(profileName).Check)
	status.Profile(profileName).CheckError(errors.New(errorMessage), progress.Summary{Duration: parseDuration("45s")}, "")
...
func TestSaveAndLoadBackupSuccess(t *testing.T) {
	...
	fs := afero.NewMemMapFs()
	status := newAferoStatus(fs, filename).Load()
	status.Profile(profileName).BackupSuccess(progress.Summary{Duration: parseDuration("1h45m")}, "")
	err := status.Save()
	assert.NoError(t, err)
...
func TestSaveAndLoadBackupError(t *testing.T) {
	...
	fs := afero.NewMemMapFs()
	status := newAferoStatus(fs, filename).Load()
	status.Profile(profileName).BackupError(errors.New(errorMessage), progress.Summary{Duration: parseDuration("1m")}, "")
	err := status.Save()
	assert.NoError(t, err)
...
func TestAddToExistingProfile(t *testing.T) {
	...
	status = newAferoStatus(fs, filename).Load()
	status.Profile(profileName).CheckSuccess(progress.Summary{Duration: parseDuration("45m")}, "")
	err = status.Save()
	assert.NoError(t, err)
...
func TestAddProfile(t *testing.T) {
	...
	fs := afero.NewMemMapFs()
	status := newAferoStatus(fs, filename).Load()
	status.Profile(profile1).BackupSuccess(progress.Summary{Duration: parseDuration("1h45m")}, "")
	err := status.Save()
	assert.NoError(t, err)

	status = newAferoStatus(fs, filename).Load()
	status.Profile(profile2).CheckSuccess(progress.Summary{Duration: parseDuration("45m")}, "")
	err = status.Save()
	assert.NoError(t, err)
...
func TestAddSuccessAfterError(t *testing.T) {
	...
	fs := afero.NewMemMapFs()
	status := newAferoStatus(fs, filename).Load()
	status.Profile(profileName).BackupError(errors.New("error message"), progress.Summary{Duration: parseDuration("45s")}, "")
	err := status.Save()
	assert.NoError(t, err)
