...
func TestEvents(t *testing.T) {
	testData := []struct {
		...
		// crontab format: minute hour dayOfMonth month dayOfWeek
		{"Sat,Thu,Mon..Wed,Sat..Sun", "00 00 * * 1,2,3,4,6,0", ""},
		{"Mon,Sun 12-*-* 2,1:23", "23 01,02 * * 1,0", ""},
		{"Wed *-1", "00 00 01 * *", "test $(date '+\\%w') -eq 3"},
		{"Wed..Wed,Wed *-1", "00 00 01 * *", "test $(date '+\\%w') -eq 3"},
		{"Wed, 17:48", "48 17 * * 3", ""},
		{"Wed..Sat,Tue 12-10-15 1:2:3", "02 01 15 10 *", "test $(date '+\\%w') -eq 2 || test $(date '+\\%w') -eq 3 || test $(date '+\\%w') -eq 4 || test $(date '+\\%w') -eq 5 || test $(date '+\\%w') -eq 6"},
		{"*-*-7 0:0:0", "00 00 07 * *", ""},
		{"10-15", "00 00 15 10 *", ""},
		{"monday *-12-* 17:00", "00 17 * 12 1", ""},
		{"sunday *-12-* 17:00", "00 17 * 12 0", ""},
		{"Mon,Fri *-*-3,1,2 *:30:45", "30 * 01-03 * *", "test $(date '+\\%w') -eq 1 || test $(date '+\\%w') -eq 5"},
		{"12,14,13,12:20,10,30", "10,20,30 12-14 * * *", ""},
		{"12..14:10,20,30", "10,20,30 12-14 * * *", ""},
		{"03-05 08:05:40", "05 08 05 03 *", ""},
		{"08:05:40", "05 08 * * *", ""},
		{"05:40", "40 05 * * *", ""},
		{"Sat,Sun 12-05 08:05:40", "05 08 05 12 *", "test $(date '+\\%w') -eq 6 || test $(date '+\\%w') -eq 0"},
	...
	for _, testRun := range testData {
		t.Run(testRun.event, func(t *testing.T) {
			...
			err = entry.Generate(buffer)
			require.NoError(t, err)
			assert.Equal(t, testRun.expected+"\t"+makeCommandLine(testRun.extra, "command line")+"\n", buffer.String())
		})
	}
}
