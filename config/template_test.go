...
func TestInfoData(t *testing.T) {
	...
	t.Run("referenceDefaults", func(t *testing.T) {
		...
		assert.Equal(t, []string{"true (config version 2)"}, fn(retention.Name(), retention.PropertyInfo(constants.ParameterHost)))

		backup := data.Profile.SectionInfo(constants.CommandBackup)
		require.NotNil(t, backup)
		assert.Equal(t, []string{"true (config version 2)"}, fn(backup.Name(), backup.PropertyInfo(constants.ParameterHost)))

		// forget has no resticprofile-applied tag/path defaults (unlike retention)
		forget := data.Profile.SectionInfo(constants.CommandForget)
		require.NotNil(t, forget)
		assert.Nil(t, fn(forget.Name(), forget.PropertyInfo(constants.ParameterTag)))
		assert.Nil(t, fn(forget.Name(), forget.PropertyInfo(constants.ParameterPath)))
	})
}
